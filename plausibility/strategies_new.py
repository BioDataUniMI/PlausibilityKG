...
def degree_aware_negative_sampling(
    ...
    type_src: str,
    type_dst: str,
    percentile: float = 0.95,          # 0.95 => top 5%
    negatives_multiplier: int = 3,
    seed: int = 42,
    max_attempts_per_needed: int = 50,
    percentuale_negativi: float = 1.0
) -> set[tuple[int, int]]:
    """
    Sample negatives only from nodes whose degree is >= the given percentile threshold
    (computed within each node type separately).

    - Filters u from src nodes in top-(1-percentile) by degree among type_src
    - Filters v from dst nodes in top-(1-percentile) by degree among type_dst
    - Adds (u,v) only if not in graph, u!=v, and labels match

    Returns a set of (u,v) negatives.
    """
    rng = random.Random(seed)
    deg = dict(g.degree())

    # nodes by type
    src_nodes = [n for n, d in g.nodes(data=True) if d.get("label") == type_src]
    dst_nodes = [n for n, d in g.nodes(data=True) if d.get("label") == type_dst]
    if not src_nodes or not dst_nodes:
        return set()

    def top_percentile_nodes(nodes: list[int]) -> list[int]:
        nodes_sorted = sorted(nodes, key=lambda n: deg.get(n, 0), reverse=True)
        k = max(1, int(len(nodes_sorted) * (1.0 - percentile)))  # top fraction size
        return nodes_sorted[:k]

    src_pool = top_percentile_nodes(src_nodes)
    dst_pool = top_percentile_nodes(dst_nodes)

    target = int(negatives_multiplier * len(pos_edges))
    if target <= 0:
        return set()
    ...
    def is_valid(u: int, v: int) -> bool:
        ...
        if g.has_edge(u, v):
            return False
        # robust guard
        if g.nodes[u].get("label") != type_src:
            return False
        if g.nodes[v].get("label") != type_dst:
            return False
        return True

    negatives: set[tuple[int, int]] = set()
    max_attempts = max_attempts_per_needed * target
    attempts = 0

    while len(negatives) < target and attempts < max_attempts:
        u = rng.choice(src_pool)
        v = rng.choice(dst_pool)
        if is_valid(u, v):
            negatives.add((u, v))
        attempts += 1
...
def degree_aware_negative_sampling(
    ...
    Parameters:
        g (nx.Graph): Input graph.
        pos_edges (list[tuple[int, int, Any]]): Positive edges (only length is used).
        type_src (str): Source node type label to consider (g.nodes[u]["label"]).
        type_dst (str): Destination node type label to consider (g.nodes[v]["label"]).
        num_buckets (int): Number of degree buckets (default=5).
        negatives_multiplier (int): #negatives = multiplier * len(pos_edges) (default=3).
        seed (int): Random seed for reproducibility.
        max_attempts_per_needed (int): Safety bound to avoid infinite loops.

    Returns:
        set[tuple[int, int]]: Set of negative edges (u, v).
    """
    rng = random.Random(seed)

    # --- Filter nodes by type (ensures type matching by construction) ---
    src_nodes = [n for n, data in g.nodes(data=True) if data.get("label") == type_src]
    dst_nodes = [n for n, data in g.nodes(data=True) if data.get("label") == type_dst]
    ...
    # --- Sort by degree (desc) inside each type ---
    src_sorted = sorted(src_nodes, key=lambda n: deg.get(n, 0), reverse=True)
    dst_sorted = sorted(dst_nodes, key=lambda n: deg.get(n, 0), reverse=True)

    # --- Bucketization helper ---
    def make_buckets(nodes_sorted: list[int], B: int) -> list[list[int]]:
        B = max(1, min(B, len(nodes_sorted)))
        buckets: list[list[int]] = [[] for _ in range(B)]
        # even-ish split by rank (not by degree thresholds)
        for idx, node in enumerate(nodes_sorted):
            b = (idx * B) // len(nodes_sorted)  # 0..B-1
            buckets[b].append(node)
        return buckets

    src_buckets = make_buckets(src_sorted, num_buckets)
    dst_buckets = make_buckets(dst_sorted, num_buckets)
    ...
    negative_edges: set[tuple[int, int]] = set()

    # --- Validity check (graph non-edge + no self-loop) ---
    def is_valid(u: int, v: int) -> bool:
        if u == v:
            return False
        if g.has_edge(u, v):
            return False
        # types already filtered, but keep as guard (robustness)
        if g.nodes[u].get("label") != type_src:
            return False
        if g.nodes[v].get("label") != type_dst:
            return False
    ...
    # --- Progressive bucket-pair ordering ---
    # Priority: start from (0,0), then pairs with small (i+j) (closer to top buckets)
    B_src = len(src_buckets)
    B_dst = len(dst_buckets)
    bucket_pairs = sorted(
        [(i, j) for i in range(B_src) for j in range(B_dst)],
        key=lambda p: (p[0] + p[1], max(p[0], p[1]))
    )

    # --- Sampling loop with safety bound ---
    needed = target
    attempts = 0
    max_attempts = max_attempts_per_needed * needed

    while len(negative_edges) < target and attempts < max_attempts:
        # iterate pairs in priority order; each iteration tries one draw from the pair
        for bi, bj in bucket_pairs:
            if len(negative_edges) >= target:
                break
...
def degree_aware_negative_sampling(
    ...
    max_attempts = 2 * max_negatives  # limite di sicurezza
    attempts = 0
    prev_size = 0
...
def shortest_path_based_negative_sampling(g: nx.Graph, pos_edges: list[tuple[int,int,any]],type_src,type_dst,percentuale_negativi = 1.0,max_distance: int =100) -> set[tuple[int, int]]:
