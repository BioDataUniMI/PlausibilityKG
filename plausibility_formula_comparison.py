...
def nrr_par_distance(dict_list: list, negative: bool):
    """
    Mean/std distance from the "wrong" end of the [0, 1] plausibility range:
    distance from 0 for negative samples (NRR), distance from 1 for blind
    positives (PAR). Lower is better in both cases.
    """
    all_values = []
    all_values_keys = {}
...
def cal_distance(pos_dict_list, neg_dict_list):
    """
    Mean/std of the combined blind-positive and negative distance errors
    (i.e. nrr_par_distance for both splits pooled together). Cal metric.
    """
    all_values = []
    all_values_keys = {}
...
def sep_distance(dict_list):
    """
    Sep metric: mean/std of how far a schema fact's plausibility score is from
    its competing alternative schema facts (same subject/object types,
    different predicate). Larger separation means the formula better
    distinguishes the target relation from its competitors.
    """
    all_values = []
    all_values_keys = {}
    for scores_dict in dict_list:
...
def sep_best_worst_rate(dict_list: list, alt: str, threshold: dict):
    """
    Sep_best/Sep_worst metric: agreement rate between the "correct" schema
    fact's plausibility score and the best (or worst) alternative competing
    schema fact, both thresholded into a binary plausible/implausible call.
    Used to check whether the correct schema fact would be selected over its
    top competitor.

    alt: 'best' or 'worst' - which competing alternative to compare against.
    threshold: per-relation threshold dict.
    """
    all_signes = []
    all_values_keys = {}
...
def score_calculation(kg, strat, negatives, formulas, relation_pairs, max_lmbda, save=True):
    """Computes and caches the non-dist blind/neg scores used by NRR/PAR/Cal."""
...
def _load_or_compute_dist(kg, strat, fr, relation_pairs, max_lmbda):
    """Loads (or computes and caches) the per-relation-pair dist scores used by Sep/Sep_best/Sep_worst."""
...
def compute_kg_metrics(kg, strategies, formulas, formulas_dist, relation_pairs, max_lmbda):
    """Computes NRR, PAR, Cal, Sep, Sep_best, Sep_worst for one bioKG."""
    ...
    for strat in strategies:
        ...
        for fr in formulas:
            ...
            _, _, NRR[strat][fr] = nrr_par_distance(list(scores_neg.values()), negative=True)
            _, _, PAR[strat][fr] = nrr_par_distance(list(scores_blind.values()), negative=False)
            _, _, Cal[strat][fr] = cal_distance(list(scores_blind.values()), list(scores_neg.values()))
    ...
    for strat in strategies:
        ...
        for fr in formulas_dist:
            ...
            _, _, Sep[strat][fr] = sep_distance(scores_list)
            # su/threshold_best/threshold_worst come from scores_used_mean, which
            # is computed regardless of the threshold passed in - a placeholder
            # 0.5 here only affects the discarded 4th return value, not su itself.
            placeholder_threshold = {rel: 0.5 for scores_dict in scores_list for rel in scores_dict.keys()}
            _, _, su[strat][fr[:-len('_dist')]], _ = sep_best_worst_rate(scores_list, 'best', placeholder_threshold)
    ...
    Sep_best, Sep_worst = {}, {}
    for strat in strategies:
        Sep_best[strat], Sep_worst[strat] = {}, {}
        for fr in formulas_dist:
            scores = _load_or_compute_dist(kg, strat, fr, relation_pairs, max_lmbda)
            scores_list = list(scores.values())
            base_fr = fr[:-len('_dist')]
            _, _, _, Sep_best[strat][fr] = sep_best_worst_rate(scores_list, 'best', cbw[strat][base_fr]['threshold_best'])
            _, _, _, Sep_worst[strat][fr] = sep_best_worst_rate(scores_list, 'worst', cbw[strat][base_fr]['threshold_worst'])

    return {
        'NRR': NRR, 'PAR': PAR, 'Cal': Cal, 'Sep': Sep,
        'Sep_best': Sep_best, 'Sep_worst': Sep_worst,
    }
...
def build_csv_results(kgs=KGS, strategies=STRATEGIES, formulas=FORMULAS):
    ...
    for kg in kgs:
        ...
        metrics = {}
        for name in ['NRR', 'PAR', 'Cal', 'Sep', 'Sep_best', 'Sep_worst']:
        ...
        for strat in strategies:
            for fr in formulas:
                ...
                for metric_name, flatten_fn, key in [
                    ('NRR', _flatten_simple, fr),
                    ('PAR', _flatten_simple, fr),
                    ('Sep', _flatten_com, fr_dist),
                    ('Sep_best', _flatten_simple, fr_dist),
                    ('Sep_worst', _flatten_simple, fr_dist),
                ]:
                ...
                try:
                    arr = _flatten_avg(metrics["PAR"][strat][fr], metrics["NRR"][strat][fr])
                    row["Cal"] = f"{round(float(np.mean(arr)), 4)} ± {round(float(np.std(arr)), 4)}"
                except (KeyError, TypeError, ValueError):
                    row["Cal"] = None
        ...
        if rows:
            fieldnames = ['view', 'strategy', 'formula', 'NRR', 'PAR', 'Cal', 'Sep', 'Sep_best', 'Sep_worst']
